...
pub struct BufReadPostArgs {
    /// The [`ActorId`] that opened the buffer.
    pub actor_id: ActorId,
}

impl<A> BufReadPost<A> {
    /// TODO: docs.
    ...
    /// Creates a new [`BufReadPost`] with the given action.
    pub fn new(action: A) -> Self {
        Self { action, buffer_id: None }
    }
}

impl<A> AutoCommand for BufReadPost<A>
where
    A: for<'ctx> Action<Args = BufReadPostArgs, Ctx<'ctx> = BufferCtx<'ctx>>,
    A::Return: Into<ShouldDetach>,
{
    ...
    fn into_callback(
        ...
        move |actor_id, ctx| {
            let args = BufReadPostArgs { actor_id };
            let buffer_id = BufferId::new(ctx.args().buffer.clone());
            let buffer_ctx = ctx
                .deref()
                .clone()
                .into_buffer(buffer_id)
                .expect("buffer was just read, so its ID must be valid");
            self.action
